...
const findAbsentUsers = async (requestedUser, companyId) => {
    try {
        ...
        const [jobTitles, clients, locations, timesheets] = await Promise.all([
            ...
            Timesheet.find({ date: todayDate }).select('userId jobTitle clientId locationId').lean(),
        ])

        const jobTitleMap = new Map(jobTitles.map(j => [j._id.toString(), j.name]))
        const clientMap = new Map(clients.map(c => [c._id.toString(), c.clientName]))
        const locationMap = new Map(locations.map(l => [l._id.toString(), l.locationName]))

        const timesheetSet = new Set(
            timesheets.map(t => `${t.userId}_${t.jobTitle}_${t.clientId || t.locationId}`)
        )
                ...
                for (const targetId of targets) {
                    ...
                    if (!timesheetSet.has(key)) {
                        ...
                        absentEntries.push({
                            ...
                            locationName: isOffice ? locationMap.get(targetId.toString()) : ""
                        })
                    }
                }
            }
    ...
    try {
        ...
        if(allowedRoles.includes(req.user.role)){
            ...
            if(req.user.role === 'Superadmin'){
                ...
                const signedTemplates = existUser?.templates.filter(template => template.isTemplateVerify ).map(template => ({
                    _id: template._id,
                    templateId: template.templateId._id,
                    templateName: template.templateId.templateName,
...
exports.getTodaysAbsentUsers = async (req, res) => {
    try {
        ...
        return res.send({ status: 500, message: "Error occurred while fetching today's absent users!" })
    }
}