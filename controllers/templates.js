  ...
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      if (!templateName || !template) {
        return res.send({
          status: 400,
          message: "Template name and template are required.",
        });
      }
      ...
      if (templateName) {
        ...
        if (existingTemplate) {
          return res.send({
            status: 409,
            message: `A template with the name ${templateName} already exists.`,
          });
        }
      }

      const requiredKeys = process.env.REQUIRED_KEY_FOR_TEMPLATE.split(",").map(
        (key) => key.replace(/^{|}$/g, "").trim().toUpperCase()
      );
      ...
      if (templateFileName.endsWith(".doc")) {
        return res.send({
          status: 400,
          message: "Ensure the uploaded file is DOCX",
        });
      }
      ...
      if (templateFileName.endsWith(".pdf")) {
        try {
          ...
          extractedKeys = extractPlaceholders(pdfData.text).map((key) =>
            key.replace(/^{|}$/g, "").trim().toUpperCase()
          );
          ...
          return res.send({
            status: 500,
            message:
              "Error parsing the PDF file. Ensure it contains selectable text.",
          });
        }
        ...
        try {
          ...
          const { value } = await mammoth.extractRawText({
            buffer: Buffer.from(template, "base64"),
          });
          ...
          extractedKeys = extractPlaceholders(value).map((key) =>
            key.replace(/^{|}$/g, "").trim().toUpperCase()
          );
          ...
          return res.send({
            status: 500,
            message:
              "Error parsing the DOCX file. Ensure it is a valid document.",
          });
        }
      }

      if (
        templateFileName.endsWith(".pdf") ||
        ...
        const extraKeys = extractedKeys.filter(
        ...
        if (extraKeys.length > 0) {
          return res.send({
            status: 400,
            message:
              `Template file contains invalid placeholders.` +
      ...
      if (template) {
        if (!document || typeof document !== "string") {
          ...
          return res.send({
            status: 400,
            message: "Invalid or missing template document.",
          });
        }
        try {
          ...
          console.error(
            "Error occurred while uploading file to AWS:",
            uploadError
          );
          return res.send({
            status: 400,
    ...
    return res.send({
      status: 500,
      message: "Error occurred while creating templates form!",
    });
  }
};
...
exports.getTemplate = async (req, res) => {
  try {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching template!",
    });
  }
};
...
exports.updateTemplate = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      let { templateName, template, templateFileName } = req.body;
      ...
      if (template && template.startsWith("data:")) {
        ...
        if (templateFileName.endsWith(".pdf")) {
          ...
          return res.send({
            status: 400,
            message: "Unsupported file format. Only PDF and DOCX are allowed.",
          });
        }
    ...
    return res.send({
      status: 500,
      message: "Something went wrong while updating template details!",
    });
  }
};
...
exports.deleteTemplate = async (req, res) => {
  try {
    ...
    return res.send({
      status: 500,
      message: "Something went wrong while removing template!",
    });
  }
};
...
exports.previewTemplate = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      const existTemplate = existUser?.templates.find(
        (temp) => temp?.templateId.toString() == templateId
      );

      const company = await Company.findOne({
        _id: existUser?.companyId.toString(),
        isDeleted: { $ne: true },
      });
      if (!company) {
        return res.send({ status: 404, message: "Company not found" });
    ...
    return res.send({
      status: 500,
      message: "Error occurred while showing template!",
    });
  }
};
...
exports.readTemplate = async (req, res) => {
  try {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while reading template!",
    });
  }
};
...
exports.saveTemplateWithSignature = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      return res.send({
        status: 200,
        message: "Signed template saved successfully",
        URL: result?.fileUrl,
      });
    } else return res.send({ status: 403, message: "Access denied" });
  } catch (error) {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while saving signature!",
    });
  }
};
  ...
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      if (!templateId) {
        return res.send({
          status: 400,
          message: "Template ID and User IDs are required.",
        });
      }
      ...
      if (!Array.isArray(userIds) || userIds.length === 0) {
        return res.send({
          status: 400,
          message: "Atleast select one user for assigned template.",
        });
      }

      const existTemplate = await Template.findOne({
        _id: templateId,
        isDeleted: { $ne: true },
      });
      if (!existTemplate) {
      ...
      const userIdsWithTemplate = usersWithTemplate.map((user) =>
        user._id.toString()
      );

      const usersToAssign = userIds.filter(
        (id) => !userIdsWithTemplate.includes(id)
      );
      ...
      return res.send({
        status: 200,
        message: "Template assigned successfully to users.",
      });
    } else return res.send({ status: 403, message: "Access denied" });
  } catch (error) {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while assigning template!",
    });
  }
};

exports.getAllUsersTemplates = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      const searchQuery = req.query.search ? req.query.search.trim() : "";
      const roleFilter = { isDeleted: { $ne: true } };

      if (companyId && companyId !== "allCompany") {
        roleFilter.companyId = new mongoose.Types.ObjectId(companyId);
      }

      if (userId && mongoose.Types.ObjectId.isValid(userId)) {
        roleFilter._id = new mongoose.Types.ObjectId(userId);
      }

      if (req.user.role === "Superadmin") {
        roleFilter.role = { $in: ["Administrator", "Manager", "Employee"] };
      } else if (req.user.role === "Administrator") {
        roleFilter.role = { $in: ["Manager", "Employee"] };
      } else if (req.user.role === "Manager") {
        roleFilter.role = "Employee";
      }

      const statusRegexMap = [
        { pattern: /^upl[a-z]*/i, value: true },
        { pattern: /^pen[a-z]*/i, value: false },
      ];

      let isTemplateUploadedFilter = null;

      for (const item of statusRegexMap) {
        if (item.pattern.test(searchQuery)) {
          isTemplateUploadedFilter = item.value;
          break;
        }
      }
      ...
      const pipeline = [
        ...
        { $unwind: "$templates" },
        ...(status === "true"
          ? [{ $match: { "templates.isTemplateUploaded": true } }]
          : []),
        ...(status === "false"
          ? [{ $match: { "templates.isTemplateUploaded": false } }]
          : []),
        {
          $addFields: {
            templateObjectId: {
              $convert: {
                input: "$templates.templateId",
                to: "objectId",
                onError: null,
                onNull: null,
              },
        ...
        {
          $unwind: { path: "$templateData", preserveNullAndEmptyArrays: true },
        },
        {
          $unwind: {
            path: "$assignedByUser",
      ...
      if (searchQuery) {
        searchMatch.push(
          ...
          {
            "templateData.templateName": { $regex: searchQuery, $options: "i" },
          },
          {
        ...
        if (isTemplateUploadedFilter !== null) {
          searchMatch.push({
            "templates.isTemplateUploaded": isTemplateUploadedFilter,
          });
        }
      }
      ...
      pipeline.push(
        {
          $project: {
            ...
            userName: {
              $cond: [
                { $ifNull: ["$personalDetails.lastName", false] },
                {
                  $concat: [
            ...
            templateURL: "$templateData.template",
            isTemplateUploaded: "$templates.isTemplateUploaded",
            uploadedURL: "$templates.templateURL",
            assignedBy: {
              $cond: [
                {
                  $ifNull: ["$assignedByUser.personalDetails.lastName", false],
                },
                {
                  $concat: [
                    "$assignedByUser.personalDetails.firstName",
                    " ",
                    "$assignedByUser.personalDetails.lastName",
                  ],
                },
                "$assignedByUser.personalDetails.firstName",
              ],
      ...
      if (searchMatch.length > 0) {
        countPipeline.push({ $match: { $or: searchMatch } });
      }

      countPipeline.push({ $count: "totalTemplates" });
      ...
      const totalTemplates = count[0]?.totalTemplates || 0;
    ...
    console.error(
      "Error occurred while fetching signd and verification pending templates:",
      error
    );
    return res.send({
      status: 500,
...
exports.uploadSignedTemplate = async (req, res) => {
  try {
    const allowedRoles = ["Administrator", "Manager", "Employee"];
    if (allowedRoles.includes(req.user.role)) {
      const { templateId, signedTemplateBase64, signedFileName } = req.body;
      const userId = req.user?._id?.toString();

      if (!signedTemplateBase64) {
        return res.send({
          status: 400,
          message: "Please upload signed template!",
        });
      }
      ...
      return res.send({
        status: 200,
        message: "Signed template upload successfully",
      });
    } else return res.send({ status: 403, message: "Access denied" });
  } catch (error) {
    console.error("Error occurred while uploading signed template:", error);
    return res.send({
      status: 500,
      message: "Error occurred while uploading signed template!",
    });
  }
};

exports.deleteSignedTemplateOfUser = async (req, res) => {
  try {
    const allowedRoles = ["Superadmin", "Administrator", "Manager"];
    if (allowedRoles.includes(req.user.role)) {
      ...
      const existSignedTemplate = existUser?.templates.find(
        (temp) =>
          temp?.templateId == templateId && temp?.isTemplateUploaded == true
      );

      if (existSignedTemplate) {
        existSignedTemplate.isTemplateUploaded = false;
        existSignedTemplate.templateURL = "";
        ...
        return res.send({
          status: 400,
          message: "Template is not signed by employee.",
        });
      }
    } else return res.send({ status: 403, message: "Access denied" });
  } catch (error) {
    console.error(
      "Error occurred while deleting signed template of employee:",
      error
    );
    return res.send({
      status: 500,
      message: "Error occurred while deleteing signed template of user!",
    });
  }
};
