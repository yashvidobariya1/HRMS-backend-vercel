...
exports.clockInFunc = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      let user_location;
      if (jobDetail?.isWorkFromOffice) {
        user_location = await Location.findOne({
          _id: locationId,
          isDeleted: { $ne: true },
        });
        if (!user_location) {
        ...
        user_location = await Client.findOne({
          _id: clientId,
          isDeleted: { $ne: true },
        });
        if (!user_location) {
      ...
      if (!location || !location.latitude || !location.longitude) {
        return res.send({
          status: 404,
          message: "Location coordinator data is not found!",
        });
      }
      ...
      if (
        ...
        return res.send({
          status: 403,
          message: "You are outside the company location area.",
        });
      }
      ...
      if (!assignedTask && req.user.role == "Employee") {
        return res.send({
          status: 404,
          message: "You don't have any tasks assigned for today!",
        });
      }
      ...
      if (checkInTime.isBefore(taskStartTime)) {
        return res.send({
          status: 400,
          message: `You can only clock in after the task time ${convertToEuropeanTimezone(
            `${assignedTask.taskDate}T${assignedTask.startTime}:00.000Z`
          ).format("LT")}.`,
        });
      }
      ...
      if (!timesheet || timesheet?.date !== currentDate) {
        ...
        if (assignedTask) {
          ...
          const taskStartTime = moment(assignedTask?.startTime, "HH:mm");
          const allowedClockInTime = moment(taskStartTime).add(
            user_location?.graceTime,
            "minutes"
          );
      ...
      const lastClockin =
        timesheet.clockinTime[timesheet.clockinTime.length - 1];

      if (lastClockin && !lastClockin.clockOut) {
        return res.send({
          status: 400,
          message: "Please clock out before clockin again.",
        });
      }
      ...
      if (existUser.role === "Employee" || existUser.role === "Manager") {
        if (jobDetail && jobDetail.assignManager) {
          const assignManager = await User.findOne({
            _id: jobDetail.assignManager,
            isDeleted: { $ne: true },
          });
          notifiedId.push(jobDetail.assignManager);
          readBy.push({
    ...
    return res.send({
      status: 500,
      message: "Something went wrong while clock in!",
    });
  }
};
  ...
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      if (!lastClockin || lastClockin.clockOut) {
        return res.send({
          status: 400,
          message: "You can't clock-out without an active clock-in.",
        });
      }
      ...
      if (!timesheet.breakTimeDeducted) {
        const [hours, minutes, seconds] = timesheet.totalHours
          .match(/\d+/g)
          .map(Number);
        const totalMinutes = hours * 60 + minutes + Math.floor(seconds / 60);

        if (totalMinutes > user_location?.breakTime) {
          duration = subtractBreakTimeFromTotalWorkingHours(
            timesheet.totalHours,
            user_location?.breakTime
          );
          timesheet.totalHours = duration;
      ...
      if (req.user.role == "Employee") {
        ...
        const [startHour, startMinute] = assignedTask?.startTime
          .split(":")
          .map(Number);
        const [endHour, endMinute] = assignedTask?.endTime
          .split(":")
          .map(Number);
        ...
        const weeklyOvertime = subtractDurations(
          totalWeeklyHours,
          `${weeklyWorkingHours}h 0m 0s`
        );
    ...
    return res.send({
      status: 500,
      message: "Something went wrong while clock out!",
    });
  }
};
...
exports.getUsersAssignClients = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      const jobDetail = existUser?.jobDetails.find(
        (job) => job._id.toString() == jobId
      );
      if (!jobDetail) {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching user's clients!",
    });
  }
};
...
exports.getUsersAssignLocations = async (req, res) => {
  try {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching user's locations!",
    });
  }
};
  ...
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      if (jobDetail?.isWorkFromOffice) {
        ...
        const location = jobDetail?.location?.find(
        ...
        const client = jobDetail?.assignClient?.find(
      ...
      if (timesheet) {
        timesheet.clockinTime = timesheet.clockinTime.map((entry) => {
          const clockInStr = entry.clockIn
    ...
    return res.send({
      status: 500,
      message: "Something went wrong while fetching the timesheet!",
    });
  }
  // try {
  //     const allowedRoles = ['Superadmin', 'Administrator', 'Manager', 'Employee'];
...
async function getTimesheetReportForViewHours(
  users,
  clientIds,
  locationIds,
  fromDate,
  ...
  const locationFilter =
    Array.isArray(locationIds) && locationIds.length > 0 ? locationIds : null;
  ...
  const clientMap = new Map(
    clientDocs.map((client) => [client?._id?.toString(), client])
  );
      ...
      if (job?._id?.toString() == TS?.jobId?.toString()) {
        for (const CI of TS?.clockinTime) {
          finalResponse.push({
            ...
            clockIn: CI?.clockIn
              ? convertToEuropeanTimezone(CI?.clockIn).format(
                  "YYYY-MM-DD HH:mm:ss"
                )
              : "",
            clockOut: CI?.clockOut
              ? convertToEuropeanTimezone(CI?.clockOut).format(
                  "YYYY-MM-DD HH:mm:ss"
                )
              : "",
  ...
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      const { startDate: fromDate, endDate: toDate } =
        getStartAndEndDateForViewHours({ startDate, endDate });
      ...
      if (isWorkFromOffice == "true") {
        if (
          ...
          (locationId == "allLocations" || locationId == "" || !locationId)
        ) {
        ...
        if (
          (userId == "allUsers" || userId == "" || !userId) &&
          (clientId == "allClients" || clientId == "" || !clientId)
        ) {
          users = await User.find(userMatch);
          userIds = users.map((user) => user._id.toString());
      ...
      const reports = await getTimesheetReportForViewHours(
        userIds,
        clientIds,
        locationIds,
        fromDate,
    ...
    return res.send({
      status: 500,
      message: "Something went wrong while fetching time sheet!",
    });
  }
  // try {
  //     const allowedRoles = ['Superadmin', 'Administrator', 'Manager', 'Employee'];
...
exports.getUsersJobLocations = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      return res.send({
        status: 200,
        message: "User's job locations fetched successfully",
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching user's job locations!",
    });
  }
};
...
exports.addTimesheetEntry = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      const jobDetail = user?.jobDetails.find(
      ...
      const taskStartTime = moment(
        `${timesheetDate} ${assignedTask.startTime}`,
        "YYYY-MM-DD HH:mm"
      );
      ...
      if (isWorkFromOffice == true && jobDetail?.isWorkFromOffice == true) {
        const existAssignLocation = jobDetail?.location.find(
          (loc) => loc?.toString() == locationId
        );
        if (!existAssignLocation) {
          return res.send({
            status: 404,
            message: "Location not assigned to this job",
          });
        }
        existTimesheet = await Timesheet.findOne({
          userId,
          jobId,
          locationId,
          date: timesheetDate,
        ...
        jobDetail?.isWorkFromOffice == false
      ) {
        const existAssignClient = jobDetail?.assignClient.find(
          (client) => client?.toString() == clientId
        );
        if (!existAssignClient) {
          return res.send({
            status: 404,
            message: "Client not assigned to this job",
          });
        }
        existTimesheet = await Timesheet.findOne({
          userId,
      ...
      if (existTimesheet && existTimesheet?.isTimerOn) {
        return res.send({
          status: 403,
          message:
            "You have already clocked in. Please clock out before starting a new session.",
        });
        ...
        const newEntry = {
          clockIn: momentTimeZone
            .tz(clockIn, "YYYY-MM-DD HH:mm", "Europe/London")
            .utc()
            .format("YYYY-MM-DDTHH:mm:ss.SSS[Z]"),
          clockOut: momentTimeZone
            .tz(clockOut, "YYYY-MM-DD HH:mm", "Europe/London")
            .utc()
            .format("YYYY-MM-DDTHH:mm:ss.SSS[Z]"),
        ...
        let totalSeconds = updatedTimesheet.clockinTime.reduce(
          (acc, entry) => acc + convertToSeconds(entry.totalTiming),
          0
        );
        ...
        if (
          !updatedTimesheet?.breakTimeDeducted &&
        ...
        let overTime = subtractDurations(
    ...
    return res.send({
      status: 500,
      message: "Error occurred while creating timesheet entry!",
    });
  }
};
...
exports.getTimesheetEntryData = async (req, res) => {
  try {
    ...
    if (allowedRole.includes(req.user.role)) {
      ...
      const jobDetail = user?.jobDetails.find(
        (job) => job._id.toString() === timesheet?.jobId.toString()
      );
      if (!jobDetail) {
        return res.send({ status: 404, message: "Job title not found" });
      }
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching timesheet data!",
    });
  }
};
...
exports.updateTimesheetEntry = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      if (userId && userId !== timesheet.userId.toString()) {
        return res.send({
          status: 400,
          message:
            "Mismatch: Selected employee does not belong to this timesheet",
        });
      }
      ...
      if (jobId && jobId !== timesheet.jobId.toString()) {
        return res.send({
          status: 400,
          message:
            "Mismatch: Selected job title does not match the original job title in this timesheet entry",
        });
      }
      ...
      if (isWorkFromOffice == "true") {
        if (locationId && locationId !== timesheet.locationId.toString()) {
          return res.send({
            status: 400,
            message:
              "Mismatch: Selected location does not match the original location in this timesheet entry",
          });
        }
      } else if (isWorkFromOffice == "false") {
        if (clientId && clientId !== timesheet.clientId.toString()) {
          return res.send({
            status: 400,
            message:
              "Mismatch: Selected client does not match the original client in this timesheet entry",
          });
        }
      }
      ...
      entry.clockIn = momentTimeZone
        .tz(clockIn, "YYYY-MM-DD HH:mm", "Europe/London")
        .utc()
        .format("YYYY-MM-DDTHH:mm:ss.SSS[Z]");
      entry.clockOut = momentTimeZone
        .tz(clockOut, "YYYY-MM-DD HH:mm", "Europe/London")
        .utc()
        .format("YYYY-MM-DDTHH:mm:ss.SSS[Z]");
      ...
      let totalSeconds = timesheet.clockinTime.reduce(
        (acc, entry) => acc + convertToSeconds(entry.totalTiming),
        0
      );
      const taskTotalHours = formatDuration(
    ...
    return res.send({
      status: 500,
      message: "Error ocurred while updating timesheet entry!",
    });
  }
};
...
exports.deleteTimesheetEntry = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      const totalHours =
        convertToSeconds(timesheet?.totalHours) -
        convertToSeconds(entry?.totalTiming);

      let overTime, isOverTime;
      if (
        convertToSeconds(timesheet?.overTime) >
        convertToSeconds(entry?.totalTiming)
      ) {
        overTime =
          convertToSeconds(timesheet?.overTime) -
    ...
    return res.send({
      status: 500,
      message: "Error occurred while deleting timesheet entry!",
    });
  }
};
...
async function getOptimizedTimesheetReport(
  users,
  clientIds,
  locationIds,
  fromDate,
    ...
    for (const doc of timesheetDocs?.timesheet) {
      ...
      for (const job of user?.jobDetails) {
        ...
        if (isWorkFromOffice == "false" && job?.isWorkFromOffice == false) {
          ...
          isWorkFromOffice == "true" &&
          job?.isWorkFromOffice == true
        ) {
          const assignedLocationIds = job?.location?.map((c) => c?.toString());
          if (!assignedLocationIds.includes(doc?.locationId?.toString()))
            continue;
        }
...
exports.getTimesheetReport = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user?.role) || req.token?.role !== "Client") {
      ...
      const { userId, clientId, locationId } = req.body;
      const { timesheetFrequency, weekDate, isWorkFromOffice, companyId } =
        req.query;
      ...
      const { startDate: fromDate, endDate: toDate } = getStartAndEndDate({
        timesheetFrequency,
        weekDate,
        startDate,
        endDate,
      ...
      const finalResponse = await getOptimizedTimesheetReport(
        userIds,
        clientIds,
        locationIds,
        fromDate,
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching timesheet report!",
    });
  }
};
...
async function getOptimizedAbsenceReport(
  users,
  clientIds,
  locationIds,
  fromDate,
  ...
  try {
    ...
    const locationDocs = await Location.find(
      locationFilter ? { _id: { $in: locationFilter } } : {}
    ).lean();
    ...
    const timesheetDocs = await Timesheet.find(
      ...
      { userId: 1, date: 1, clientId: 1, locationId: 1 }
    ).lean();
    ...
    timesheetDocs.forEach((doc) => {
      const id =
        isWorkFromOffice === "false"
          ? `${doc.userId}_${doc.clientId}_${moment(doc.date).format(
              "YYYY-MM-DD"
            )}`
          : `${doc.userId}_${doc.locationId}_${moment(doc.date).format(
              "YYYY-MM-DD"
            )}`;
      timesheetSet.add(id);
    });
    ...
    for (const user of usersData) {
      ...
      for (const job of jobList) {
        ...
        if (isWorkFromOffice === "false" && job?.isWorkFromOffice === false) {
          const assignedClientIds = (job.assignClient || []).map((id) =>
            id.toString()
          );
        ...
        if (isWorkFromOffice === "true" && job?.isWorkFromOffice === true) {
          const assignedLocationIds = (job.location || []).map((id) =>
            id.toString()
          );
...
exports.getAbsenceReport = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user?.role)) {
      ...
      const finalResponse = await getOptimizedAbsenceReport(
        userIds,
        clientIds,
        locationIds,
        fromDate,
...
exports.downloadTimesheetReport = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user?.role) || req.token?.role === "Client") {
      ...
      let jobDetail = user?.jobDetails.find(
        (job) => job._id.toString() === jobId
      );
      if (!jobDetail) {
        return res.send({ status: 404, message: "JobTitle not found" });
      ...
      for (
        ...
        d.isSameOrBefore(endMoment, "day");
        d.add(1, "day")
      ) {
      ...
      const reportData = allDates
        .map((date) => {
          ...
          if (timesheetEntry) {
            status = "Present";
            clockinTime = timesheetEntry.clockinTime
              .map((t) => moment(t.clockIn, "HH:mm").format("h:mm A"))
              .join(" || ");
            clockoutTime = timesheetEntry.clockinTime
              .map((t) =>
                t.clockOut ? moment(t.clockOut, "HH:mm").format("h:mm A") : "-"
              )
              .join(" || ");
            ...
            status =
              leaveEntry.selectionDuration === "First-Half" ||
              leaveEntry.selectionDuration === "Second-Half"
                ? "Half Leave"
                : "Leave";
      ...
      const weeklySummary = Array.from(weeklyData, ([weekKey, data]) => {
        ...
        return {
          weekRange: `${startDate.format("DD-MM-YYYY")} to ${endDate.format(
            "DD-MM-YYYY"
          )}`,
      ...
      const fileName = `${user?.personalDetails.firstName}${
        user?.personalDetails.lastName
      }_timesheet_${
      ...
      if (format === "pdf") {
        ...
        const templatePath = path.join(
          __dirname,
          "../views/timesheetReportFormat.ejs"
        );
        ...
        reportData.forEach((row) => {
          ...
          const clockInArray = row.clockinTime
            .split("||")
            .map((time) => time.trim());
          const clockOutArray = row.clockoutTime
            .split("||")
            .map((time) => time.trim());
          ...
          const startRowNumber = worksheet.lastRow
          ...
          if (numRows > 1) {
            worksheet.mergeCells(
              `A${startRowNumber}:A${startRowNumber + numRows - 1}`
            );
            worksheet.mergeCells(
              `C${startRowNumber}:C${startRowNumber + numRows - 1}`
            );
            worksheet.mergeCells(
              `D${startRowNumber}:D${startRowNumber + numRows - 1}`
            );
            worksheet.mergeCells(
              `E${startRowNumber}:E${startRowNumber + numRows - 1}`
            );
            worksheet.mergeCells(
              `F${startRowNumber}:F${startRowNumber + numRows - 1}`
            );
          }
        });
        ...
        return res.send({
          status: 400,
          message: "Invalid format. Please specify 'pdf' or 'excel'.",
        });
      }
    } else return res.send({ status: 403, message: "Access denied" });
  } catch (error) {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while downloading timesheet report!",
    });
  }
};
  ...
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      if (companyId && companyId !== "allCompany") {
        ...
        query.companyId = new mongoose.Types.ObjectId(
      ...
      return res.send({
        status: 200,
        message: "All users and clients fetched successfully",
        clients,
        users,
      });
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching users and clients",
    });
  }
};
...
exports.getAllClientsOfUser = async (req, res) => {
  try {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching users all clients!",
    });
  }
};
...
exports.getAllUsersOfClientOrLocation = async (req, res) => {
  try {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while fetching users by client!",
    });
  }
};
  ...
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      if (report?.status !== "Pending") {
        return res.send({
          status: 403,
          message: `Report is already action by ${report?.actionBy}`,
        });
      }
                            ...
                            <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
                                ...
                                <div style="padding: 30px;">
                                    ...
                                    <p style="font-size: 16px; color: #333333;">
                                        ...
                                        <strong>${moment(startDate).format(
                                          "DD-MM-YYYY"
                                        )}</strong> to 
                                        <strong>${moment(endDate).format(
                                          "DD-MM-YYYY"
                                        )}</strong>.
                                    </p>
                                    <p style="font-size: 16px; color: #333333;">
        ...
        transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
            if (error.code == "EENVELOPE") {
              console.warn(
                "Invalid email address, while re-generating report link:",
                email
              );
            } else {
              console.error(
                "Error while sending re-generating report link:",
                error
              );
            }
          }
      ...
      return res.send({
        status: 200,
        message: "Client report re-generated successfully.",
        data,
      });
    } else return res.send({ status: 403, message: "Access denied" });
  } catch (error) {
    ...
    return res.send({
      status: 500,
      message: "Error occurred while re-generate report link!",
    });
  }
};
