...
exports.getAllLoggedInOutUsers = async (req, res) => {
  try {
    ...
    if (allowedRoles.includes(req.user.role)) {
      ...
      const users = await LoginAudit.find(baseQuery)
        .populate("userId", "personalDetails role")
        .skip(skip)
        .limit(limit);
      const formattedUsers =
        users.length > 0
          ? users.map((user) => ({
              ...
              reason: user.reason || "",
              attemptTime:
                convertToEuropeanTimezone(user?.attemptTime).format(
                  "YYYY-MM-DD HH:mm:ss"
                ) || "",
              userName:
                `${
                  user?.userId?.personalDetails?.lastName
                    ? `${user?.userId?.personalDetails?.firstName} ${user?.userId?.personalDetails?.lastName}`
                    : `${user?.userId?.personalDetails?.firstName}`
                }` || "",
              // lastTimeLoggedIn: convertToEuropeanTimezone(user?.lastTimeLoggedIn).format("YYYY-MM-DD HH:mm:ss") || "",
              lastTimeAccess:
                convertToEuropeanTimezone(user?.lastTimeAccess).format(
                  "YYYY-MM-DD HH:mm:ss"
                ) || "",
    ...
    console.error(
      "Error occurred while fetching logged In/Out employees:",
      error
    );
    return res.send({
      status: 500,
      message: "Error occurred while fetching logged In/Out employees!",
    });
  }
};
