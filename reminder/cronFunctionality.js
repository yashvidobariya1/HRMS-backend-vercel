    ...
    try {
        ...
        for(const employee of allEmployees){
            ...
            for(const job of jobDetails){
                ...
                const timesheet = await Timesheet.findOne({ userId: _id, jobId: job?._id, date: today, isDeleted: { $ne: true } })
                ...
                for(const task of tasks){
                    ...
                    if (type === 'clock-in' && !hasClockIn) {
                        ...
                        await Notification.create({
                            ...
                            message: `You haven't clocked in today for '${job?.jobTitle}' Role.`,
                    ...
                    if (type === 'clock-out' && hasMissingClockOut && isTimerOn) {
                        ...
                        await Notification.create({
                            ...
                            message: `You haven't clocked out yet for '${job?.jobTitle}' Role.`,
                            readBy: [{ userId: _id, role: employee?.role }]
                        })
                        await delay(1000)
                    }
